...
//For casting void * to type int and long long
typedef union
{
	void *ptr;
	int* val;
...
int main(void) {
	...
	//flags for checking return value of functions
	int get_table_num;
	...
	//unique order ID's for table, online and reservation order
		...
		if (0 == rcvid) {
			...
			switch (client_msg.pulse.code) {
				case PERIODIC_TIMER_PULSE_CODE:
					...
					if (calculate_current_hour(dateTime.tm_hour) == 0 && dateTime.tm_min == 0) {
						...
						//Get reservations for current day from database
			...
			switch(client_msg.type) {
				case CREATE_RESERVATION_MSG_TYPE:
					...
					//If reservation successful, build the reply message to send to client
					if (create_reservation_flag > 0) {
						...
						//If an error occured, return -1 to client
				...
				case CREATE_TABLE_ORDER_MSG_TYPE:
					...
					//If the table order failed, reset all of the menu counts
					...
					//Something went wrong when creating table order item structs
					else {
						MsgReply(rcvid, 0, &create_table_order_flag, sizeof(create_table_order_flag));
						printf("table order failed\n");
					}
				...
				case GET_PRINT_RECEIPT_MSG_TYPE:
					...
					//There exists table orders for the specified table number
...
int create_reservation(int num_seats, char *meta_tag, int year, int month, int day, char *fName, char *lName, long long phoneNum, int start_hour, int start_min, int duration_hour, int duration_min, int curYear, int curMonth, int curDay, int resID)
{
	...
	int found_table = 0;
	int flag = 0;
	//Check whether the reservation is for current day
		...
		while (cpyNumSeats <= MAX_NUM_SEATS) {
			...
			for (int i = 0; i < tables_size; i++) {
				...
				if (tables[i].isReserved == 0 && tables[i].num_seats == cpyNumSeats) {
					//Search for table with the correct table meta-tag (patio, bar, party, etc.)
					for(int k = 0; k < MAX_TABLE_TAGS; k++) {
						...
						if ((strcmp(table_tags[k].meta_tag_name, meta_tag) == 0 && table_tags[k].table_num == tables[i].table_num) || (strcmp(meta_tag,"") == 0)) {
							...
							//Check if the table will be reserved during the requested reservation time
			...
			//Look for a table with more seats
			cpyNumSeats++;
		}
		...
		while (cpyNumSeats <= MAX_NUM_SEATS) {
			for (int i = 0; i < tables_size; i++) {
				...
				if (tables[i].num_seats == cpyNumSeats) {
					//Found a table, search for the table meta tag
					for(int k = 0; k < MAX_TABLE_TAGS; k++) {
						//printf("table num: %d, table num seats: %d, table tag: %s\n", tables[i].table_num, tables[i].num_seats, table_tags[k].meta_tag_name);
						if ((strcmp(table_tags[k].meta_tag_name, meta_tag) == 0 && table_tags[k].table_num == tables[i].table_num) || (strcmp(meta_tag, "") == 0)) {
							printf("table num: %d, table num seats: %d, table tag: %s\n", tables[i].table_num, tables[i].num_seats, table_tags[k].meta_tag_name);
							//Check if reservations are made for that table on the specified date and time
							...
							//Did not find a table, keep searching
							if (flag == 1) {
								flag = 0;
								break;
							}
...
float create_online_order(int year, int month, int day, int hour, int minute, char *fName, char *lName, char *address, long long phoneNum, int curYear, int curMonth, int curDay, int orderID)
{
	...
	//Check if online order is for current day
	if (curYear == year && curMonth == month && curDay == day) {
		online_orders_today[num_online_orders_today++] = makeOnlineOrder;
	}
	//Check if online order is for a future date
	else {
		online_orders_future[num_online_orders_future++] = makeOnlineOrder;
	}
	//Print all the online orders, online order items, and menu items
	for (int i = 0; i < num_online_orders_today; i++) {
...
int create_table_order(int table_num, int orderID) {
	//Iterate through menu item counts for calculating the total
	for (int i = 0; i < MAX_MENU_ITEMS; i++) {
		if (menu[i].count > 0) {
			//Create table order item structs
			printf("menu[i]: %s\n", menu[i].name);
			...
			//Increment count variable for checking if there are multiple orders of the same menu item
			for (int j = 0; j < menu[i].count; j++) {
	...
	//Print all the existing table orders (for debugging pruposes)
	for (int i = 0; i < num_existing_orders; i++) {
...
int get_table_in_house(int num_people) {
	...
	while (num_seats <= MAX_NUM_SEATS) {
		...
		for (int i = 0; i < tables_size; i++) {
			//Check if the number of seats for the table is equal to num_seats. Also check if the table is currently reserved
			if (tables[i].num_seats == num_seats && tables[i].isReserved == 0) {
				//If table is found, set the table to reserved and return the table number
				tables[i].isReserved = 1;
				return tables[i].table_num;
			}
		}
		//If no tables with specified seat number is found, increment the number of seats and continue searching
		num_seats++;
	}
	//No tables were found, return -1
	return -1;
}

float table_print_receipt(int table_num) {
	float total = 0;
	//Iterate through table order item struct array
	for (int i = 0; i < num_existing_orders; i++) {
		//If the table number is found in the table order items
		if (table_orders_existing[i].table_num == table_num) {
			//Iterate through all menu items
			for (int j = 0; j < MAX_MENU_ITEMS; j++) {
				//compare the menu item name with the menu item name in table order item struct
				if (strcmp(table_orders_existing[i].menu_item_name, menu[j].name) == 0) {
					//Multiple orders of the same menu item
	...
	//If total is 0, no table order items were found with the specified table number, return 0
	if (total == 0) {
		return 0;
	}
	//If total > 0, return the total
	return total;
}
...
//Calculate current year properly
int calculate_current_year(int year) {
	return year + 1900;
}

//Calculate current month properly
int calculate_current_month(int month) {
	return month+1;
}

//Insert tables into database and table array of structs
int insert_tables_database(qdb_hdl_t *hdl) {
	int numSeats = 0;
